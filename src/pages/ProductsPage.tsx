...
export default function ProductsPage() {
  ...
  const handleAddToOrder = (lineItem: OrderLineItem) => {
    ...
    // On mobile (<1024px) show a toast instead of opening the cart drawer
    // so users can keep adding more products without interruption.
    const isMobile = window.innerWidth < 1024;
    if (isMobile) {
      if (toastTimerRef.current) clearTimeout(toastTimerRef.current);
      setToast(`${lineItem.productName} added ✓`);
  ...
  return (
    <div className="pt-24 min-h-screen bg-brand-cream relative">
      ...
      {/* Mobile Toast — shows briefly after adding a product, so users can keep browsing */}
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            className="fixed bottom-[140px] left-1/2 -translate-x-1/2 z-[9999] lg:hidden"
          >
            <div className="flex items-center gap-2 bg-brand-charcoal text-white text-[12px] font-bold tracking-wide px-4 py-3 rounded-full shadow-2xl border border-white/10">
