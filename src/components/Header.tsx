...
function DropdownPanel({
  ...
  return (
    ...
    >
      {/* Top accent bar */}
      <div className="h-[3px] w-full bg-gradient-to-r from-micmag-red via-brand-red-deep to-brand-red" />
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <span className={`flex items-center gap-1.5 text-[11.5px] font-black text-brand-charcoal ${item.color.hoverText} transition-colors font-sans`}>
                  {item.label}
...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[100] flex flex-col w-full font-sans">
      <motion.nav
        id="navbar"
        className={`w-full transition-all duration-500 relative z-[110] ${
      ...
      >
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-[0.12] z-0" style={{ borderRadius: '18px' }}>
          <motion.div
            ...
            className="absolute -top-12 left-[15%] w-40 h-40 rounded-full bg-micmag-red filter blur-[40px]"
          />
          <motion.div
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <Link to="/" className="flex items-center gap-2.5 group flex-shrink-0">
            ...
            <div className="flex flex-col text-left">
              ...
              <span className="text-[8.5px] font-sans font-bold text-micmag-red uppercase tracking-widest mt-0.5 opacity-90">Homes & Fittings</span>
            </div>
          </Link>
          ...
          <ul className="hidden lg:flex items-center gap-7 xl:gap-9 list-none m-0 p-0 h-10">
              ...
              <li key={link.name} className="relative h-full flex items-center">
                <div className="relative group/link py-1 flex items-center">
                  <Link
                    ...
                    className="text-[11px] xl:text-[0.79rem] font-bold tracking-[0.12em] uppercase transition-colors duration-300 hover:text-micmag-red"
                    style={{ color: '#1c1917' }}
                  >
                    {link.name}
                  </Link>
            ...
            >
              <div className="relative group/link py-1 flex items-center gap-1 cursor-pointer">
                ...
                <ChevronDown className={`w-3 h-3 transition-transform duration-300 ${openDropdown === 'about' ? 'rotate-180 text-micmag-red' : 'text-neutral-400'}`} />
                <span className="absolute bottom-[-4px] left-0 w-0 h-[2px] bg-gradient-to-r from-amber-600 via-orange-500 to-micmag-red transition-all duration-300 group-hover/link:w-full" />
              </div>
            ...
            >
              <div className="relative group/link py-1 flex items-center gap-1 cursor-pointer">
                ...
                <ChevronDown className={`w-3 h-3 transition-transform duration-300 ${openDropdown === 'contact' ? 'rotate-180 text-micmag-red' : 'text-neutral-400'}`} />
                <span className="absolute bottom-[-4px] left-0 w-0 h-[2px] bg-gradient-to-r from-amber-600 via-orange-500 to-micmag-red transition-all duration-300 group-hover/link:w-full" />
              </div>
          ...
          <div className="hidden lg:flex items-center gap-3">
            <a
              ...
              className="flex items-center gap-1.5 text-[11px] font-bold uppercase hover:text-micmag-red transition-colors duration-200"
              style={{ color: '#1c1917' }}
            >
              <PhoneCall className="w-3.5 h-3.5 text-micmag-red" />
        ...
        {isMenuOpen && (
          <>
            ...
            <motion.div
              ...
              className="lg:hidden fixed inset-y-0 right-0 z-[105] w-full max-w-[320px] h-[100dvh] shadow-2xl pt-24 px-6 flex flex-col justify-between pb-8"
              style={{ 
            ...
            >
              <div className="space-y-1 text-left overflow-y-auto pb-4 no-scrollbar">
                    ...
                    <div className="py-2 border-b border-neutral-200">
                      <Link to="/#about" onClick={(e) => { closeMenu(); handleLinkClick('/#about', e); }}
                        className="flex items-center justify-between text-sm font-black tracking-[0.08em] uppercase text-brand-charcoal hover:text-micmag-red transition-colors pb-2">
                        <span>About</span>
                      </Link>
                      <div className="pl-3 border-l-2 border-brand-charcoal/10 space-y-0.5">
                        {ABOUT_ITEMS.map(item => (
                          <Link key={item.href} to={item.href} onClick={(e) => { closeMenu(); handleLinkClick(item.href, e); }}
                            className="flex items-center gap-2 py-1.5 text-[11px] font-bold uppercase text-brand-charcoal/70 hover:text-micmag-red tracking-wider transition-colors">
                  ...
                  <div className="space-y-3 mt-auto pt-4 shrink-0">
                    <div className="border-t border-neutral-200 pt-4 text-left">
                      <div className="text-[10px] font-mono uppercase text-brand-charcoal/60 tracking-wider">Direct line</div>
                      <a href="tel:+2347052940445" className="text-sm font-black text-brand-charcoal hover:text-micmag-red transition-colors inline-block mt-0.5">
                        +234 (0) 705 294 0445
                      </a>
                    </div>
                    <Link
